...
namespace OutCode.EscapeTeams.ObjectRepository.Hangfire.Entities
{
    internal class ServerModel : ModelBase
    {
        ...
        {
            get => _server.Data;
            set => UpdateProperty(_server, () => x => _server.Data, value);
        }

        public DateTime LastHeartbeat
        {
            get => _server.LastHeartbeat;
            set => UpdateProperty(_server, () => x => _server.LastHeartbeat, value);
        }

        public string Name
        {
            get => _server.Name;
            set => UpdateProperty(_server, () => x => _server.Name, value);
        }
    }
}
