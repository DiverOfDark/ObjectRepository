...
namespace OutCode.EscapeTeams.ObjectRepository.Hangfire.Entities
{
    internal class HashModel : ModelBase
    {
        ...
        {
            get => _hash.ExpireAt;
            set => UpdateProperty(_hash, () => x => x.ExpireAt, value);
        }

        public string Field
        {
            get => _hash.Field;
            set => UpdateProperty(_hash, () => x => x.Field, value);
        }

        public string Key
        {
            get => _hash.Key;
            set => UpdateProperty(_hash, () => x => x.Key, value);
        }

        public string Value
        {
            get => _hash.Value;
            set => UpdateProperty(_hash, () => x => x.Value, value);
        }
    }
}
